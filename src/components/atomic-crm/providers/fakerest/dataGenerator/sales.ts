import { internet, name, phone, random } from "faker/locale/en_US";
...
export const generateSales = (_: Db): Sale[] => {
  const randomSales = Array.from(Array(5).keys()).map((id) => {
    ...
    return {
      ...
      calendario: "",
      linea_negocio: random.arrayElement(["exclusiva", "compartida"] as const),
      activo: true,
      // Advisors report to the admin (id 0) in the demo dataset.
      manager_id: 0,
    };
  });
  return [
